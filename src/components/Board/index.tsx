    ...
    <div>
      {playerID === ctx.currentPlayer ? (
        <button onClick={() => moves.endTurn()}>
          End Turn ({G.state.actionsRemaining}/{MAX_ACTIONS_PER_TURN} actions
          remaining)
        </button>
      ) : (
        ""
            ...
            <div>
              ...
              <CardList>
                    ...
                    {set.map((card, cardIndex) => {
                      ...
                      switch (stage) {
                        case STAGE_MOVE_PROPERTY:
                          moveFunctions["Add"] = () =>
                            moves.moveProperty(setIndex);
                          break;
                        default:
                          if (ctx.currentPlayer === id) {
                            moveFunctions["Move"] = () =>
                              moves.moveProperty(setIndex, cardIndex);
                            if (card.colour.length === 2) {
                              moveFunctions["Flip"] = () =>
                                moves.flipWildcardProperty(setIndex, cardIndex);
                            }
                          }
                          break;
                      }
                ...
                {stage === STAGE_MOVE_PROPERTY ? (
                  <button
                    onClick={() =>
                      moves.moveProperty(value.properties.length, null)
                    }
                  >
                    New Set
                  </button>
